...
class TestRightOnlyRegressions(unittest.TestCase):
    def test_rightonly_wraps_dict_value_replacement(self):
        source = (1, [False])
        expected_diff = {replace: {}}
        actual_diff = diff(source, {}, syntax="rightonly")
        self.assertEqual(expected_diff, actual_diff)

    def test_rightonly_set_full_change_returns_rhs_set(self):
        self.assertEqual(set(), diff({1, 2}, set(), syntax="rightonly"))
